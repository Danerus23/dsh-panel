...
| `--onboard` | 再次打开首次启动向导 |
| `--status [--out 文件]` | 打印服务器状态、费率、余额、开机启动项指向哪里以及备份状态 |
| `--env-check [--out 文件]` | 打印面板找到的东西：node、`dsh` 包、密钥存储、端口 |
| `--autostart-fix [--out 文件]` | 核对开机启动项是否指向此副本并修复（面板每次启动时也会这样做） |
...
备份写完后可以校验：在备份窗口点「校验」、用 `--backup-check`，或在恢复时校验 —— 会比对文件数量
和前 64 MB 的校验和。写完不会自动校验。如果打包了密钥，这个归档就是机密，不要外传。

恢复之前，面板会在备份目录旁边写入 `dsh-before-restore-<日期>.zip`（只保留最近三个），并停掉自己
的服务器 —— DSH 文件不能被占用 —— 然后把同名文件放回原处，其余内容保持不动。从 `keys/` 恢复的
目录会限制为仅所有者（owner）和 SYSTEM 可访问。
