...
- **Три языка.** Русский, English и 中文 — по языку системы, переключаются в «Настройках»
  и применяются сразу.
- **Обновления.** Панель обновляет себя с выпусков GitHub: скачивает свою сборку, требует
  опубликованный `SHA256SUMS.txt` и показывает, что нового.
...
архив ничего не ставит — он только создаёт папки данных ниже и, если попросить, запись автозапуска.
Данные лежат в папках ниже; удаление программы (или
удаление через «Программы и компоненты») убирает саму панель, а папки данных остаются, пока их не
удалишь сам: удаляется только то, что панель создаёт заново, — папка обновлений и свой переносимый
Node внутри папки состояния.
...
Скрипт собирает панель, прогоняет все автоматические проверки, которые может, собирает установщик и
складывает файлы выпуска в `dist/`. Три проверки в него намеренно не входят — `tools\check-installed.ps1`
(ставит панель), `tools\check-onboarding.ps1` (нужен рабочий стол) и `tools\verify-release.ps1`
(ходит в GitHub); какие именно и почему — в `AGENTS.md`. Кроме исходников нужны: .NET SDK 8, Node.js (для проверок, которые работают с заглушкой
...
| Данные DSH — твои: панель их читает и пишет в них только при накате копии | `%USERPROFILE%\.dsh` |
...
| `--status [--out файл]` | напечатать состояние сервера, тариф и баланс, куда ведёт запись автозапуска и состояние копий |
| `--env-check [--out файл]` | напечатать, что панель нашла: node, пакет `dsh`, хранилище с ключом, порт |
| `--autostart-fix [--out файл]` | сверить запись автозапуска с этой копией и починить её (то же панель делает при каждом старте) |
...
| `--hidden` | то же, что `--tray`: запустить панель свёрнутой в трей (псевдоним для записей автозапуска, оставшихся от старых версий) |
...
Копию можно проверить после записи: кнопкой «Проверить» в окне копий, ключом `--backup-check` или
при накате — сверяются число файлов и первые 64 МБ контрольных сумм. Автоматической проверки сразу
после записи нет. Если ключи упакованы — архив является секретом, передавать его нельзя.

Перед накатом панель кладёт рядом с копиями `dsh-before-restore-<дата>.zip` (хранятся три последние),
останавливает свой сервер — файлы DSH не должны быть заняты — и возвращает файлы с теми же именами на
место, всё остальное оставляя как есть. Каталоги, восстановленные из `keys/`, закрываются на владельца
и SYSTEM.
...
![Четыре окна разом: панель, настройки, копии и меню трея.](docs/demo.png)

Четыре окна разом: панель, настройки, копии и меню трея.
