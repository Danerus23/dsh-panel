...
/// молча отбирать у соседа автозапуск нельзя, — но говорим о ней человеку пунктом меню и
/// подсказкой галочки в окне панели (шарик про такую запись не показывается, см.
/// <see cref="ShouldNotify"/>).
///
...
public static class Autostart
{
    ...
    /// <summary>
    /// Показывать ли шарик про автозапуск. Два правила, оба выстраданы:
    /// 1) изолированный прогон (<paramref name="isolatedRun"/> — это <see cref="IsIsolatedRun"/>:
    ///    подменены каталоги панели или ветка реестра) не показывает НИЧЕГО — он не трогает
    ///    машинные записи и не должен пугать человека сообщениями про его же реестр (наши проверки
    ///    запускают собранную копию панели на его машине);
    /// 2) состояние «запись ведёт на живую соседнюю копию» шариком не сообщаем: оно и так видно в
    ///    меню трея («включено для другой копии») и подсказкой галочки в окне панели
    ///    (<c>MainForm.SetAutostartNote</c>), а человек от такого шарика только недоумевает —
    ///    исправлять там нечего. Подсказка значка (<c>TrayHost.BuildTooltip</c>) про автозапуск
    ///    не говорит вовсе: в ней версия, сервер, тариф и баланс.
    /// Шарик остаётся там, где человек сам ничего не увидит: запись ведёт на отсутствующий файл и
    /// починить не удалось, либо реестр не читается.
    /// </summary>
    public static bool ShouldNotify(Where state, bool isolatedRun)
    {
        if (isolatedRun) return false;
        return state is Where.Missing or Where.Unknown;
    }
